...
const defaultHighlightPills = [
  ...
  "Parent & Student Mobile PWA",
  "Real-Time Attendance SMS",
  "Multi-Tenant Cloud",
];

const defaultHeroStats: { label: string; value: string }[] = [
  { label: "Schools Onboarded", value: "500+" },
  { label: "Students Empowered", value: "1,00,000+" },
  { label: "Fee Recovery Rate", value: "99.4%" },
];
...
export interface GlowyWavesHeroProps {
  ...
  highlightPills?: readonly string[] | string[];
...
export function GlowyWavesHero({
  badge = "Next-Gen Intelligent School ERP",
  title = "Smart School Management.",
  titleGradient = "Empower Education.",
  subtitle = "Streamline student admissions, mobile attendance, automated fee collection with Razorpay, period timetables, and parent communication in one ultra-fast, modern cloud system.",
  ctaPrimaryText = "Get Started Free",
  ctaPrimaryHref = "/register",
  onCtaSecondaryClick,
  ctaSecondaryText = "Watch Demo",
  highlightPills = defaultHighlightPills,
        ...
        >
          ...
          >
            <Link href={ctaPrimaryHref}>
              <Button
                size="lg"
                className="group gap-2 rounded-full px-8 text-base font-bold shadow-lg shadow-blue-500/25 uppercase tracking-[0.15em]"
              >
                {ctaPrimaryText}
                <ArrowRight
          ...
          <motion.ul
            variants={itemVariants}
            className="mb-12 flex flex-wrap items-center justify-center gap-3 text-xs font-semibold uppercase tracking-[0.15em] text-foreground/70 dark:text-foreground/80"
          >
          ...
          >
            {stats.map((stat) => (
              <motion.div
                ...
                className="space-y-1"
              >
                <div className="text-xs uppercase tracking-[0.2em] text-foreground/50 dark:text-foreground/60">
                  {stat.label}
                </div>
