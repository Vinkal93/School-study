...
export function NavigationItem({ item, unreadCount = 0 }: NavigationItemProps) {
  ...
  return (
    <Link
      ...
      className={`relative flex-1 min-w-[52px] sm:min-w-[60px] py-1.5 rounded-full flex flex-col items-center justify-center transition-all duration-200 active:scale-90 outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 ${
        isActive
    ...
    >
      <div className="relative flex items-center justify-center">
        ...
        />
        {badgeLabel && (
          <span className="absolute -top-1 -right-2 min-w-[14px] h-3.5 bg-red-500 text-white text-[9px] font-black rounded-full flex items-center justify-center px-1 shadow-xs border border-white dark:border-slate-900">
            {badgeLabel}
          </span>
        )}
      </div>
      <span className={`text-[11px] leading-tight mt-1 truncate max-w-full px-1 ${
