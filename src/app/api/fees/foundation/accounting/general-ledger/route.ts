...
import { getGeneralLedger } from "@/lib/services/accounting.service";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const statement = await getGeneralLedger(targetSchoolId, accountId, {
      academicYearId,
      startDate,
      endDate,
    });
    ...
    console.error("GET /api/fees/foundation/accounting/general-ledger error:", err);
    return NextResponse.json({ error: err.message || "Failed to load General Ledger" }, { status: 500 });
  }
}
