...
import { getProfitAndLossStatement } from "@/lib/services/accounting.service";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const statement = await getProfitAndLossStatement(targetSchoolId, {
      academicYearId,
      startDate,
      endDate,
    });
    ...
    console.error("GET /api/fees/foundation/accounting/profit-and-loss error:", err);
    return NextResponse.json(
      { error: err.message || "Failed to generate Profit & Loss statement" },
      { status: 500 }
    );
  }
}
